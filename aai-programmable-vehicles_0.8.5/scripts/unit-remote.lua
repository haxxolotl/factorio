...
function UnitRemote.show_gui(player)
    if player.gui.left.remote_selected_units == nil then
        ...
        if storage.player_selected_units and storage.player_selected_units[player.index] then
            for _, selected_unit in pairs(storage.player_selected_units[player.index]) do
                local unit = selected_unit.unit
                local unit_type = unit_get_type(unit)
                local prototype = prototypes.entity[unit_type.vehicle_whole]
...
function UnitRemote.deselect_unit(player, remove_unit_id)
    ...
    if not storage.player_selected_units then
        storage.player_selected_units = {}
    elseif storage.player_selected_units[player.index] then
        local selected_units = storage.player_selected_units[player.index]
...
function UnitRemote.deselect_units(player)
    ...
    elseif storage.player_selected_units[player.index] then
        ...
        storage.player_selected_units[player.index] = nil
    end
end
...
function UnitRemote.select_units(player, units)
  ...
  storage.player_selected_units[player.index] = {}
  for _, unit in pairs(units) do
      storage.player_selected_units[player.index][unit.unit_id] = {unit = unit, selection = nil}
    ...
    if (event.item == "unit-remote-control") then
        ...
        if alt then
            if storage.restrict_remote_actions_to_player_ids then
              if not Util.table_contains(storage.restrict_remote_actions_to_player_ids, event.player_index) then
                player.print({"aai-programmable-vehicles.restricted-remote-action"})
                return
              end
            end
            -- move_to
            if storage.player_selected_units and storage.player_selected_units[event.player_index] then
                local selected_units = {} -- make array
                for _, s in pairs(storage.player_selected_units[event.player_index]) do
                ...
                if follow_target then
                  ...
                  -- not following a unit. Maybe assign to a path start.
                    ...
                    and math.abs(area.right_bottom.y - area.left_top.y) < follow_max_selection_box then
                      ...
                      if storage.forces
                       ...
                       and storage.forces[force_name].surface_paths[surface_index].paths then
                        for path_id, path in pairs(storage.forces[force_name].surface_paths[surface_index].paths) do
...
function UnitRemote.save_unit_group(player, name)
  storage.player_selected_units = storage.player_selected_units or {}

  local unit_sel = storage.player_selected_units[player.index]
  ...
  if not storage.unit_selection_groups then storage.unit_selection_groups = {} end
  if not storage.unit_selection_groups[player.index] then storage.unit_selection_groups[player.index] = {} end
  ...
  storage.unit_selection_groups[player.index][name] = selection
end

-- Loads the saved selection
-- (per-player)
function UnitRemote.load_unit_group(player, name)
  if not (storage.unit_selection_groups and storage.unit_selection_groups[player.index]) then
    return UnitRemote.deselect_units(player)
  end

  local unit_sel = storage.unit_selection_groups[player.index][name]
