...
remote.add_interface(
    ...
    {
        write_global = function() helpers.write_file("AAI/vehicles.storage.lua", serpent.block(storage, {comment=false, sparse=true, nocode=true}), false) end,
        write_unit = function(data)
          local unit = unit_find_from_entity(data.entity)
          helpers.write_file("AAI/vehicles."..data.entity.unit_number..".lua", serpent.block(unit, {comment=false, sparse=true, nocode=true}), false)
        end,
        get_units = function() return storage.unit.units end, -- returns table of units by unit id
