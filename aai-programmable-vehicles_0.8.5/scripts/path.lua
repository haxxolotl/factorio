...
function Path.get_force_paths(force_name)
  ...
  if not storage.forces[force_name] then storage.forces[force_name] = {} end
  if not storage.forces[force_name].surface_paths then storage.forces[force_name].surface_paths = {} end
  return storage.forces[force_name].surface_paths
end

function Path.get_surface_paths(force_name, surface_index)
  if storage.forces
    and storage.forces[force_name]
    and storage.forces[force_name].surface_paths
    and storage.forces[force_name].surface_paths[surface_index]
    and storage.forces[force_name].surface_paths[surface_index].paths then
      return storage.forces[force_name].surface_paths[surface_index].paths
  end
end
...
function Path.get_path(force_name, surface_index, path_id)
  if storage.forces
    ...
    and storage.forces[force_name].surface_paths
    and storage.forces[force_name].surface_paths[surface_index]
    and storage.forces[force_name].surface_paths[surface_index].paths
    and storage.forces[force_name].surface_paths[surface_index].paths[path_id] then
      return storage.forces[force_name].surface_paths[surface_index].paths[path_id]
  end
end
...
function Path.new(force_name, surface_index, path_id)
  ...
  storage.forces[force_name].surface_paths = storage.forces[force_name].surface_paths or {}
  storage.forces[force_name].surface_paths[surface_index] = storage.forces[force_name].surface_paths[surface_index] or {}
  storage.forces[force_name].surface_paths[surface_index].paths = storage.forces[force_name].surface_paths[surface_index].paths or {}
  local path = {
    type = "path",
  ...
  storage.forces[force_name].surface_paths[surface_index].paths[path_id] = path
  return path
end
...
function Path.clear(force_name, surface_index, path_id)
  ...
  if path then
    ...
    storage.forces[force_name].surface_paths[surface_index].paths[path_id] = nil
  end
end
...
function Path.player_get_active_path_surface_index(player)
  if storage.players and storage.players[player.index] and storage.players[player.index].selected_path_surface_index then
    return storage.players[player.index].selected_path_surface_index
  end
end
...
function Path.player_set_active_path_surface_index(player)
  ...
  if storage.players[player_index].selected_path_surface_index ~= surface_index then
    storage.players[player_index].selected_path_surface_index = surface_index
    storage.players[player_index].selected_path_id = nil
    storage.players[player_index].selected_waypoint_id = nil
  end
end

function Path.player_get_active_path_id(player)
  if storage.players and storage.players[player.index] and storage.players[player.index].selected_path_id then
    return storage.players[player.index].selected_path_id
  end
end
...
function Path.player_set_active_path_id(player, path_id, waypoint_id)
  ...
  if storage.players[player_index].selected_path_surface_index ~= surface_index then
    storage.players[player_index].selected_path_surface_index = surface_index
    storage.players[player_index].selected_path_id = path_id
    storage.players[player_index].selected_waypoint_id = waypoint_id
  end
  if storage.players[player_index].selected_path_id ~= path_id then
    storage.players[player_index].selected_path_id = path_id
    storage.players[player_index].selected_waypoint_id = waypoint_id
  end
  if waypoint_id then
...
function Path.player_get_active_path_waypoint_id(player)
  if storage.players and storage.players[player.index] and storage.players[player.index].selected_waypoint_id then
    local surface_index = player.surface.index
    if player.surface.index ~= storage.players[player.index].selected_path_surface_index then
      storage.players[player.index].selected_waypoint_id = 1
    end
    return storage.players[player.index].selected_waypoint_id
  end
end
  ...
  if not path.visualisation or table_size(path.visualisation.player_indexes) == 0 then
    ...
    if not path.visualisation.player_indexes[player_index] then
      ...
      for _, marker in pairs(path.visualisation.markers) do
        if not (path.show_start and _ == path.first_waypoint_id) and marker.valid then
          marker.players = path.visualisation.player_indexes
        end
      end
      for _, text in pairs(path.visualisation.text) do
        if not (path.show_start and _ == path.first_waypoint_id) and text.valid then
          text.players = path.visualisation.player_indexes
        end
      end
      for _, line in pairs(path.visualisation.lines) do
        if line.valid then
          line.players = path.visualisation.player_indexes
        end
      end
    end
  end
...
function Path.set_path_waypoint_data(data)
  ...
  if data and data.force_name and data.surface_index and data.path_id then
      ...
      if data.waypoint_id then
          ...
          waypoint.direct_drive_through = data.direct_drive_through or false
