...
function Navpath.indicator_clear(unit)
  ...
  if navpath_indicator.final then
    navpath_indicator.final.destroy()
  end
...
function Navpath.indicator_draw_direct(unit)
  ...
  if navpath_indicator.final and navpath_indicator.final.valid then
    navpath_indicator.final.target = unit.safe_target_position
  else
  ...
  if not (navpath_indicator.lines[1] and navpath_indicator.lines[1].valid) then
    ...
    navpath_indicator.lines[1].from = unit.vehicle
    navpath_indicator.lines[1].to = unit.safe_target_position
  end
...
function Navpath.indicator_draw_navpath(unit)
  ...
  for i = 1, navpath.current_index-1, 1 do
    if navpath_indicator.lines[i] then
      navpath_indicator.lines[i].destroy()
  ...
  for i = navpath.current_index, #navpath.waypoints, 1 do
    ...
    if source and target then
      if navpath_indicator.lines[i] and navpath_indicator.lines[i].valid then
        navpath_indicator.lines[i].from = source
        navpath_indicator.lines[i].to = target
      else
  ...
  if navpath_indicator.final and navpath_indicator.final.valid then
    navpath_indicator.final.target = final_point
  else
