      ...
      for i = 1, max_waypoint_id + 1 do
        ...
        if i == selected_waypoint_id then
          ...
          if waypoint_type == "position" then
            ...
            local direct_drive_through = inner.add{
              type = "flow",
              name = "direct_drive_through",
              direction = "horizontal",
            }

            direct_drive_through.add{
              type = "checkbox",
              name = "direct_drive_through_checkbox",
              state = waypoint.direct_drive_through or false,
              tooltip = {"aai-programmable-vehicles.waypoint_direct_drive_through_tooltip"}
            }

            direct_drive_through.add{
              type = "label",
              name = "direct_drive_through_label",
              caption = {"aai-programmable-vehicles.waypoint_direct_drive_through_caption"},
              tooltip = {"aai-programmable-vehicles.waypoint_direct_drive_through_tooltip"}
            }
          end
          ...
          if waypoint_type == "goto_unit_id"
            ...
            if waypoint_type == "goto_vehicle_depot_id" then waypoint_entity_id_flow.input_label.caption = "Vehicle Depot ID" end
...
function PathRemote.on_gui_text_changed(event)
  ...
  if gui_element_or_parent(element, "aai_path_gui") then
    ...
    elseif element.name == "speed_limit_input" then
        ...
        local force_name = player.force.name
        local path = Path.get_or_create(force_name, player.surface.index, path_id)
        path.speed_limit = (Util.string_to_number(element.text or "0") or 0) / 1000
        PathRemote.show_gui(player)
      return
    end
  end
    ...
    if element.name == "dropdown_box" and element.parent.name == "waypoint_type" then
      ...
      if selected_value == "position" then
          ...
          direct_drive_through.add{
            type = "checkbox",
            name = "direct_drive_through_checkbox",
            state = waypoint and waypoint.direct_drive_through or false,
            tooltip = {"aai-programmable-vehicles.waypoint_direct_drive_through_tooltip"}
          }
...
function PathRemote.on_gui_click(event)
  ...
  if gui_element_or_parent(element, "aai_path_gui") then
    if not (storage.players[player_index].selected_path_surface_index and player.surface.index == storage.players[player_index].selected_path_surface_index) then
    ...
    elseif element.name == "loop_path" or element.parent.name == "loop_path" then
      ...
      Path.update_visualisation(path)
      PathRemote.show_gui(player)
    elseif element.name == "direct_drive_through" or element.name == "direct_drive_through_label" then
      local direct_drive_through = element.name == "direct_drive_through" and element or element.parent
      direct_drive_through.direct_drive_through_checkbox.state = not direct_drive_through.direct_drive_through_checkbox.state
    ...
    elseif element.name == "waypoint_save" then
        ...
        if waypoint_type == "position" then
          ...
          local direct_drive_through = false
          if waypoint_element.waypoint_inner.direct_drive_through and waypoint_element.waypoint_inner.direct_drive_through.direct_drive_through_checkbox then
            direct_drive_through = waypoint_element.waypoint_inner.direct_drive_through.direct_drive_through_checkbox.state
          end
          Path.add_waypoint(force_name, player.surface.index, path_id, {
            ...
            direct_drive_through = direct_drive_through
          }, waypoint_id)
        elseif waypoint_type == "goto_position_beacon_id"
          ...
          local target_id = tonumber(waypoint_element.waypoint_inner.waypoint_entity_id.input_textfield.text)
