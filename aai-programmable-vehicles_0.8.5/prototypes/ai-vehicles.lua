...
local function make_composite_unit_from_vehicle(vehicle)
    ...
    if vehicle.type == "car" then
      ...
      solid.hidden = true
      solid.factoriopedia_alternative = vehicle.name
      ...
      ghost.order = "z-z"
      ghost.hidden = true
      ghost.factoriopedia_alternative = vehicle.name
    ...
    data:extend{
        {
            ...
            collision_mask = vehicle.collision_mask or { layers = {item = true, player = true, water_tile = true}},
...
function aai_make_ai_vehicles(vehicle)
  ...
  a_entity.rotation_snap_angle = 0
  ...
  for _, recipe in pairs(data.raw.recipe) do
    if    recipe.results
      ...
      and recipe.results[1].name
      and recipe.results[1].name == a_item.name
    then
      a_recipe = recipe
      break
    end
  ...
  for i = 1, #valid_guns, 1 do
    ...
    local original_icons = vehicle.icons or {{ icon = vehicle.icon, icon_size = vehicle.icon_size or 64}}
    local item_icons = {}
    ...
    local a_localised_name = a_entity.localised_name or {"entity-name.".. a_entity.name}
    ...
    local b_recipe = {
      ...
      category = "crafting",
      ingredients = {
        {type = "item", name = a_item.name, amount = 1},
        {type = "item", name = "electronic-circuit", amount = 1}
      },
      results = {
        {type = "item", name = b_item.name, amount = 1},
      },
      enabled = false,
    ...
    local b_recipe_reverse = {
      ...
      localised_name = {"remove-ai", a_localised_name},
    ...
    if b_gun == 0 or #valid_guns == 1 then
      b_item.localised_name = {"split-vehicle", a_localised_name}
      b_entity.localised_name = {"split-vehicle", a_localised_name}
    else
      b_entity.guns = { b_gun }
      b_item.localised_name = {"split-vehicle-with", a_localised_name, {"item-name."..b_gun}}
      b_entity.localised_name = {"split-vehicle-with", a_localised_name, {"item-name."..b_gun}}
    end
