...
signal_for_surface = {type = "virtual", name = "signal-surface", quality="normal", comparator="="} ---@as SignalFilter.0
signal_for_follow_id = {type = "virtual", name = "signal-follow-id", quality="normal", comparator="="} ---@as SignalFilter.0
signal_for_follow_player = {type = "virtual", name = "signal-follow-player", quality="normal", comparator="="} ---@as SignalFilter.0
signal_for_position_beacon_id = {type = "virtual", name = "signal-position-beacon-id", quality="normal", comparator="="} ---@as SignalFilter.0
signal_for_vehicle_depot_id = {type = "virtual", name = "signal-vehicle-depot-id", quality="normal", comparator="="} ---@as SignalFilter.0
signal_for_position_x_tiles = {type = "virtual", name = "signal-x-tile", quality="normal", comparator="="} ---@as SignalFilter.0 -- x tiles
signal_for_position_y_tiles = {type = "virtual", name = "signal-y-tile", quality="normal", comparator="="} ---@as SignalFilter.0 -- y tiles
signal_for_position_x_sub = {type = "virtual", name = "signal-x-sub", quality="normal", comparator="="} ---@as SignalFilter.0 -- * 100 x tiles
signal_for_position_y_sub = {type = "virtual", name = "signal-y-sub", quality="normal", comparator="="}  ---@as SignalFilter.0 -- * 100 y tiles

signal_for_path = {type = "virtual", name = "signal-path", quality="normal", comparator="="} ---@as SignalFilter.0
signal_for_waypoint = {type = "virtual", name = "signal-waypoint", quality="normal", comparator="="} ---@as SignalFilter.0
...
on_entity_deployed
raise_event implementation: raise_event('on_entity_deployed', {entity = LuaEntity, signals = {signaltype={signalname={signalquality={signal="", count=#}}}}})
...
function fuel_w (item)
  if item and item.valid and item.fuel_value then
    if item.fuel_value == math.huge then
      return math.huge
    end
        ...
        if item and item.valid then
          if item.fuel_value and fuel_w (item)>0 then
            ...
            if high_fuel_item[item.fuel_category] then
              if fuel_w(prototypes.item[high_fuel_item[item.fuel_category]]) < fuel_w(item) then
          ...
          if item.type=="ammo" then
            ...
            if ammo_type and item.ammo_category then
                if ammo_lst[item.ammo_category] then
                  ammo_lst[item.ammo_category][item.name]=item
                else
                  ammo_lst[item.ammo_category]={[item.name]=item}
                end
    ...
    total_fuel_value_from_contents = function(contents, fuel_categories)
      ...
      for _, item_stack in pairs(contents) do
        if item_stack then
          ...
          for category, enabled in pairs(fuel_categories) do
            if enabled and item_stack and fuel_items[category] and fuel_items[category][item_name] then
              total = total + fuel_items[category][item_name].fuel_value * item_count
            end
          end
        end
      end
...
function unit_setup_vehicle(vp) -- vehicle prototype
    ...
    if vp.type == "car" and not (
      prototypes.entity[vp.name .. composite_suffix .. "solid"]
      and prototypes.entity[vp.name .. composite_suffix .. "ghost"]
      and prototypes.entity[vp.name .. composite_suffix .. "navigator"]
      and prototypes.entity[vp.name .. composite_suffix .. "driver"]
      and prototypes.entity[vp.name .. composite_suffix .. "buffer"] ) then
        return
    end
    if vp.type == "spider-vehicle" and not (
      prototypes.entity[vp.name .. composite_suffix .. "navigator"]
      and prototypes.entity[vp.name .. composite_suffix .. "driver"]
      and prototypes.entity[vp.name .. composite_suffix .. "buffer"] ) then
        return
    end
    ...
    if vp.collision_mask and vp.collision_mask.layers then
        if vp.collision_mask.layers['ground_tile'] then
            unit_type.collides_with_ground = true
        elseif ( not vp.collision_mask.layers['object'] )
          and ( not vp.collision_mask.layers['player'] )
          then
    ...
    if storage.hauler_types[vp.name] then
    ...
    storage.unit_types[unit_type.name] = unit_type
    storage.unit_types_by_signal[Util.signal_to_string(unit_type.signal)] = unit_type
end
...
function struct_find_from_entity(entity)
  if storage.structure_unit_numbers and storage.structure_unit_numbers[entity.unit_number] then
    return storage.structures[storage.structure_unit_numbers[entity.unit_number]]
  end
end
...
function unit_load_prototypes()
    ...
    storage.unit_types = {}
    storage.unit_types_by_signal = {}
    storage.unit_mineable_resources = {}
    storage.hauler_types = {}
    storage.hauler_types["vehicle-hauler-0"] = "vehicle-hauler-0"
    storage.hauler_types["vehicle-warden-electro-bolter-gun"] = "vehicle-warden-electro-bolter-gun"
    storage.hauler_types["cargo-plane-0"] = "cargo-plane-0"
    storage.hauler_types["compicar-0"] = "compicar-0"
    storage.hauler_types["hovercraft-entity-0"] = "hovercraft-entity-0"
    storage.hauler_types["hcraft-entity-0"] = "hcraft-entity-0"

    for _, response_types in pairs(raise_event('hauler_types', nil)) do
        for _, response_type in pairs(response_types) do
            storage.hauler_types[response_type] = response_type
        end
    end
        ...
        if prototype.type == "resource" and prototype.mineable_properties and prototype.mineable_properties.products then
          for _, product in pairs(prototype.mineable_properties.products) do
            if product.type == "item" then
                storage.unit_mineable_resources[product.name] = product.name
            end
          end
        end
    end
...
function unit_get_type(unit)
    return storage.unit_types[unit.unit_type]
end
...
function unit_has_energy(unit)
    if unit.vehicle and unit.vehicle.valid then
      if    (not unit.vehicle.burner)
        or  unit.vehicle.burner.remaining_burning_fuel > 0
        or  (not unit.vehicle.burner.inventory.is_empty())
      then
        return true
      end
...
function unit_on_destroy_entity(entity)
    if entity.valid and entity.unit_number then
        storage.unit.unit_numbers[entity.unit_number] = nil
    end
end
...
function unit_find_from_entity(entity)
    local unit_id = storage.unit.unit_numbers[entity.unit_number]
    if unit_id then
        return storage.unit.units[unit_id]
    end
    return nil
end

function unit_by_surface_type_index(surface_index, unit_type, index)
    if storage.unit.surface_units_by_type[surface_index] and storage.unit.surface_units_by_type[surface_index][unit_type] and #storage.unit.surface_units_by_type[surface_index][unit_type] > 0 then
        --index = unit_loop_index(unit_type, index)
        if index > 0 and index <= #storage.unit.surface_units_by_type[surface_index][unit_type] then
            return storage.unit.surface_units_by_type[surface_index][unit_type][index]
        elseif index < 0 and -index <= #storage.unit.surface_units_by_type[surface_index][unit_type] then
            return storage.unit.surface_units_by_type[surface_index][unit_type][#storage.unit.surface_units_by_type[surface_index][unit_type] + index + 1]
        end
    end
end
...
function unit_by_unit_number(unit_number)
    return storage.unit.unit_numbers[unit_number]
end
...
function unit_find_from_signal(data)
    ...
    if signal_count and signal_count.signal and signal_count.count then
        if signal_count.signal.name == "signal-id" then
            ...
            local unit_type = storage.unit_types_by_signal[Util.signal_to_string(signal_count.signal)]
            if unit_type then
...
function unit_set_data(data)
    ...
    if unit_id and storage.unit.units[unit_id] and storage.unit.units[unit_id].vehicle
    and storage.unit.units[unit_id].vehicle.valid then
        storage.unit.units[unit_id].data = signal_data
    end
end
...
function rebuild_units_of_type_table(units_of_type)
  if not units_of_type then return end
  local valid = {}
  for i, unit in ipairs(units_of_type) do
    if unit.vehicle and unit.vehicle.valid then
      table.insert(valid, unit)
      unit.unit_type_id = #valid
    end
  end
  return valid
end

function update_surface_unit_type_ids(surface_index, unit_type_name)
  if storage.unit.surface_units_by_type and
    storage.unit.surface_units_by_type[surface_index] then
      local surface_units_by_type = storage.unit.surface_units_by_type[surface_index]
      surface_units_by_type[unit_type_name] = rebuild_units_of_type_table(surface_units_by_type[unit_type_name])
  end
end

function update_all_surfaces_unit_type_ids(unit_type_name)
  for _, surface_units_by_type in pairs(storage.unit.surface_units_by_type) do
    surface_units_by_type[unit_type_name] = rebuild_units_of_type_table(surface_units_by_type[unit_type_name])
  end
end
...
function update_all_unit_type_ids()
  for _, surface_units_by_type in pairs(storage.unit.surface_units_by_type) do
    for unit_type_name, units_of_type in pairs(surface_units_by_type) do
      surface_units_by_type[unit_type_name] = rebuild_units_of_type_table(units_of_type)
    end
  end
end
...
function unit_get_count_by_surface_and_type(surface_index, unit_type)
  if storage.unit.surface_units_by_type and
    storage.unit.surface_units_by_type[surface_index] and
    storage.unit.surface_units_by_type[surface_index][unit_type] then
    return #storage.unit.surface_units_by_type[surface_index][unit_type]
  end
  return 0
end
...
function unit_manage_new(entity, event, signals)
  ...
  for _, unit_type in pairs(storage.unit_types) do
    if entity.name == unit_type.vehicle_whole or entity.name == unit_type.vehicle_solid or entity.name == unit_type.vehicle_ghost then
        local unit_id = storage.unit.next_unit_id
        storage.unit.next_unit_id = storage.unit.next_unit_id + 1
        ...
        if unit_type.is_miner then
            for _, resource in pairs(storage.unit_mineable_resources) do
                if resource ~= "wood" and resource ~= "coal" then
                    Util.signal_container_add(data, {value={type="item", name=resource, quality="normal", comparator="="}, min=-1})
                end
            end
            Util.signal_container_add(data, {value={type="item", name="wood", quality="normal", comparator="="}, min=50})
            Util.signal_container_add(data, {value={type="item", name="coal", quality="normal", comparator="="}, min=100})
            Util.signal_container_add(data, {value={type="item", name="solid-fuel", quality="normal", comparator="="}, min=50})
            if prototypes.item["processed-fuel"] then
              Util.signal_container_add(data, {value={type="item", name="processed-fuel", quality="normal", comparator="="}, min=400})
            end
        elseif string.find(unit.unit_type, "vehicle-warden", 1, true) then
            Util.signal_container_add(data, {value={type="item", name="repair-pack", quality="normal", comparator="="}, min=100})
            Util.signal_container_add(data, {value={type="item", name="coal", quality="normal", comparator="="}, min=50})
            Util.signal_container_add(data, {value={type="item", name="solid-fuel", quality="normal", comparator="="}, min=50})
            Util.signal_container_add(data, {value={type="virtual", name="signal-minimum-fuel", quality="normal", comparator="="}, min=800})
            if prototypes.item["processed-fuel"] then
              Util.signal_container_add(data, {value={type="item", name="processed-fuel", quality="normal", comparator="="}, min=400})
            end
        elseif unit_type.is_hauler then
            for _, resource in pairs(storage.unit_mineable_resources) do
                Util.signal_container_add(data, {value={type="item", name=resource, quality="normal", comparator="="}, min=12000})
            end
            Util.signal_container_add(data, {value={type="item", name="wood", quality="normal", comparator="="}, min=12000})
            Util.signal_container_add(data, {value={type="item", name="solid-fuel", quality="normal", comparator="="}, min=12000})
            Util.signal_container_add(data, {value={type="virtual", name="signal-minimum-fuel", quality="normal", comparator="="}, min=800})
            if prototypes.item["processed-fuel"] then
              Util.signal_container_add(data, {value={type="item", name="processed-fuel", quality="normal", comparator="="}, min=400})
            end
            ...
            Util.signal_container_add(data, {value={type="item", name="solid-fuel", quality="normal", comparator="="}, min=50})
            if prototypes.item["processed-fuel"] then
              Util.signal_container_add(data, {value={type="item", name="processed-fuel", quality="normal", comparator="="}, min=200})
            end
        end
            ...
            for _, item in pairs(prototypes.item) do
                if item.type == "ammo" then
                    ...
                    if ammo_type and ammo_type.category == ammo_category then
                        Util.signal_container_add(data, {value={type = "item", name=item.name, quality="normal", comparator="="}, min=50})
                    end
                end
            end
          end
        ...
        if signals then
            ...
            for type, names in pairs(unit.data) do
                for name, qualities in pairs(names) do
                    if name ~= unit_type.name then
                        signals_valid = true
                        break
                    end
                end
        ...
        storage.unit.units[unit_id] = unit
        storage.unit.unit_numbers[entity.unit_number] = unit_id
        if not storage.unit.surface_units_by_type[surface_index] then
          storage.unit.surface_units_by_type[surface_index] = {}
        end
        if not storage.unit.surface_units_by_type[surface_index][unit.unit_type] then
          storage.unit.surface_units_by_type[surface_index][unit.unit_type] = {}
        end
        table.insert(storage.unit.surface_units_by_type[surface_index][unit.unit_type], unit)
        update_surface_unit_type_ids(surface_index, unit.unit_type)
        storage.unit.entities_pending_manage[entity.unit_number] = nil

        if signals then
          local path_id
          local waypoint_id
          if    signals[signal_for_path.type]
            and signals[signal_for_path.type][signal_for_path.name]
            and signals[signal_for_path.type][signal_for_path.name][signal_for_path.quality]
            and signals[signal_for_path.type][signal_for_path.name][signal_for_path.quality][signal_for_path.comparator]
          then
            path_id = signals[signal_for_path.type][signal_for_path.name][signal_for_path.quality][signal_for_path.comparator].min
          end
          if    signals[signal_for_waypoint.type]
            and signals[signal_for_waypoint.type][signal_for_waypoint.name]
            and signals[signal_for_waypoint.type][signal_for_waypoint.name][signal_for_waypoint.quality]
            and signals[signal_for_waypoint.type][signal_for_waypoint.name][signal_for_waypoint.quality][signal_for_waypoint.comparator]
          then
            waypoint_id = signals[signal_for_waypoint.type][signal_for_waypoint.name][signal_for_waypoint.quality][signal_for_waypoint.comparator].min
          end
          if path_id then
...
function unit_unmanage(unit)
    if unit then
        ...
        ---@cast selection_graphic LuaRenderObject
        if unit.selection_graphic and unit.selection_graphic.valid then
          unit.selection_graphic.destroy()
        end
        if unit.selection_graphic and unit.selection_graphic.valid then
          unit.selection_graphic.destroy()
        end
        Navpath.indicator_clear(unit)
        storage.unit.units[unit.unit_id] = nil

        update_all_surfaces_unit_type_ids(unit.unit_type)
...
function unit_create_entity(unit, entity_type, surface, position, force, quality)
  local safe_pos = position
  local entity = surface.create_entity{name=entity_type, position=safe_pos, force=force, quality=quality}
  if not entity then
    ...
    entity = surface.create_entity{name=entity_type, position=safe_pos, force=force, quality=quality}
  end
  entity.teleport(position)
  if unit and entity.unit_number then
      storage.unit.unit_numbers[entity.unit_number] = unit.unit_id
  end
  return entity
end
...
function unit_create_entity_from_entity(unit, entity_type, source_entity, replace)
  if not source_entity.valid then
    storage.unit.unit_numbers[source_entity.unit_number] = nil
    return
  end
  ...
  local entity = unit_create_entity(unit, entity_type, source_entity.surface, source_entity.position, source_entity.force, source_entity.quality)
  ...
  if replace then
    ...
    Util.copy_logistic_requests(source_entity, entity)

    -- this MUST happen before the other events are triggered.
    if unit.vehicle == source_entity then
      unit.vehicle = entity
    end

    if source_entity.logistic_cell and source_entity.logistic_cell.logistic_network and #source_entity.logistic_cell.logistic_network.robots > 0 then
      storage.robot_collections = storage.robot_collections or {}
      table.insert(storage.robot_collections, {vehicle = entity, robots = source_entity.logistic_cell.logistic_network.robots})
    end
...
function struct_get_position_beacon_id(force_name, surface_index, beacon_id)
  if storage.forces
    and storage.forces[force_name]
    and storage.forces[force_name].surfaces
    and storage.forces[force_name].surfaces[surface_index]
    and storage.forces[force_name].surfaces[surface_index].position_beacons
    and storage.forces[force_name].surfaces[surface_index].position_beacons[beacon_id]
    and storage.forces[force_name].surfaces[surface_index].position_beacons[beacon_id].structs then
      for _, struct in pairs(storage.forces[force_name].surfaces[surface_index].position_beacons[beacon_id].structs) do
...
function struct_remove_from_position_beacon_id(force_name, surface_index, struct_id, beacon_id)
  if storage.forces
    ...
    and storage.forces[force_name].surfaces[surface_index].position_beacons[beacon_id]
    and storage.forces[force_name].surfaces[surface_index].position_beacons[beacon_id].structs then
      storage.forces[force_name].surfaces[surface_index].position_beacons[beacon_id].structs[struct_id] = nil
      if table_size(storage.forces[force_name].surfaces[surface_index].position_beacons[beacon_id].structs) == 0 then
        -- remove position beacon id if empty
        storage.forces[force_name].surfaces[surface_index].position_beacons[beacon_id] = nil
      end
  end
end
...
function update_position_beacon_from_entity (entity)
  ...
  storage.forces[force_name] = storage.forces[force_name] or {}
  storage.forces[force_name].surfaces = storage.forces[force_name].surfaces or {}
  storage.forces[force_name].surfaces[surface.index] = storage.forces[force_name].surfaces[surface.index] or {}
  storage.forces[force_name].surfaces[surface.index].position_beacons = storage.forces[force_name].surfaces[surface.index].position_beacons or {}
  local target_table = storage.forces[force_name].surfaces[surface.index].position_beacons

  -- get beacon id
  local beacon_id = nil
  local control = entity.get_or_create_control_behavior()
  ---@cast control LuaConstantCombinatorControlBehavior
  local logi_section = control.get_section(1)
  local filter = logi_section.filters[1]
  if    filter
    and filter.value
    and filter.value.name == signal_for_position_beacon_id.name
    and filter.min
  then
    beacon_id = filter.min
  end
  ...
  logi_section.set_slot(1, {value = signal_for_position_beacon_id, min = beacon_id})
  logi_section.set_slot(2, {value = signal_for_position_x_tiles, min = math.floor(entity.position.x)})
  logi_section.set_slot(3, {value = signal_for_position_y_tiles, min = math.floor(entity.position.y)})
  logi_section.set_slot(4, {value = signal_for_position_x_sub, min = math.floor(entity.position.x * 100)})
  logi_section.set_slot(5, {value = signal_for_position_y_sub, min = math.floor(entity.position.y * 100)})
  logi_section.set_slot(6, {value = signal_for_surface, min = entity.surface.index})
...
function struct_get_vehicle_depot_id(force_name, surface_index, depot_id)
  if storage.forces
    ...
    and storage.forces[force_name].surfaces
    and storage.forces[force_name].surfaces[surface_index]
    and storage.forces[force_name].surfaces[surface_index].vehicle_depots
    and storage.forces[force_name].surfaces[surface_index].vehicle_depots[depot_id]
    and storage.forces[force_name].surfaces[surface_index].vehicle_depots[depot_id].structs then
      for _, struct in pairs(storage.forces[force_name].surfaces[surface_index].vehicle_depots[depot_id].structs) do
        return struct -- return first
      end
  end
end
...
function struct_remove_from_vehicle_depot_id(force_name, surface_index, struct_id, depot_id)
  if storage.forces
    ...
    and storage.forces[force_name].surfaces[surface_index].vehicle_depots[depot_id]
    and storage.forces[force_name].surfaces[surface_index].vehicle_depots[depot_id].structs then
      storage.forces[force_name].surfaces[surface_index].vehicle_depots[depot_id].structs[struct_id] = nil
      if table_size(storage.forces[force_name].surfaces[surface_index].vehicle_depots[depot_id].structs) == 0 then
        -- remove vehicle depot id if empty
        storage.forces[force_name].surfaces[surface_index].vehicle_depots[depot_id] = nil
      end
  end
end
...
function update_vehicle_depot_from_entity (entity)
  ...
  storage.forces[force_name].surfaces = storage.forces[force_name].surfaces or {}
  storage.forces[force_name].surfaces[surface.index] = storage.forces[force_name].surfaces[surface.index] or {}
  storage.forces[force_name].surfaces[surface.index].vehicle_depots = storage.forces[force_name].surfaces[surface.index].vehicle_depots or {}
  local target_table = storage.forces[force_name].surfaces[surface.index].vehicle_depots

  -- get depot id
  local depot_id = nil
  local control = struct.sub.combinator.get_or_create_control_behavior()
  ---@cast control LuaConstantCombinatorControlBehavior
  local logi_section = control.get_section(1)
  local filter = logi_section.filters[1]
  if    filter
    and filter.value
    and filter.value.name == signal_for_vehicle_depot_id.name
    and filter.min
  then
    depot_id = filter.min
  end
  ...
  logi_section.set_slot(1, {value = signal_for_vehicle_depot_id, min = depot_id})
  logi_section.set_slot(2, {value = signal_for_position_x_tiles, min = math.floor(entity.position.x)})
  logi_section.set_slot(3, {value = signal_for_position_y_tiles, min = math.floor(entity.position.y)})
  logi_section.set_slot(4, {value = signal_for_position_x_sub, min = math.floor(entity.position.x * 100)})
  logi_section.set_slot(5, {value = signal_for_position_y_sub, min = math.floor(entity.position.y * 100)})
...
function unit_update_mode(unit)
    ...
    if unit_type.is_flying and (unit.mode == "unit" or unit.mode == "vehicle_move_to_temp") then
        unit.mode = "vehicle_move_to" -- don't need navigator for fliers
        unit.stuck = 0
...
function consume_fuel_or_equipment (unit)
    ...
    if unit.vehicle.grid and unit.vehicle.grid.available_in_batteries > 10000 and unit.vehicle.grid.available_in_batteries > unit.vehicle.grid.battery_capacity * 0.5 then
      ...
      if #high_fuel_item <1 then storage.fuel_items = get_fuel.build() end
      ...
      for cat, enbl in pairs (unit.vehicle.burner.fuel_categories) do
        if enbl then
          ...
          if high then
            local proto = prototypes.item[high]
            if proto then
              if fuel == "" or proto.fuel_value > prototypes.item[fuel].fuel_value then
      ...
      unit.vehicle.burner.currently_burning = prototypes.item[fuel]
      local energy_deficit = prototypes.item[fuel].fuel_value - unit.vehicle.burner.remaining_burning_fuel

      local batteries = Util.table_filter(unit.vehicle.grid.equipment, function(v) return v.type == "battery-equipment" end)
      for _, battery in pairs(batteries) do
        ...
        if energy_deficit <= 0 then break end
      end
...
function unit_load_ammo (unit) -- return true for has ammo
  ...
  if inv_ammo.is_empty() then
    ...
    if inv_trunk then
      for _, item_quality_count in pairs(inv_trunk.get_contents()) do
        if inv_ammo.can_insert(item_quality_count) then
          local stack = inv_trunk.find_item_stack({name = item_quality_count.name, quality = item_quality_count.quality})
          inv_ammo[1].transfer_stack(stack)
          return true
        end
      end
    end
    ...
    if unit_load_ammo(unit) then
      ...
      if (not unit.attack_target) and (game.tick + unit.unit_id % unit_target_search_interval) then
        unit.attack_target = unit.vehicle.surface.find_nearest_enemy{
          ...
          max_distance = unit_type.gun.attack_parameters.range,
          force = unit.vehicle.force}
        if settings.global["aai-attack-target-fallback"].value == true and (not unit.attack_target) then
          unit.attack_target = unit.vehicle.surface.find_nearest_enemy_entity_with_owner{
            position = unit.vehicle.position,
            max_distance = unit_type.gun.attack_parameters.range,
            force = unit.vehicle.force}
        end
...
function count_inventories_items(inventories, item_name, quality)
    local _quality = quality or "normal"
    local count = 0
    for _, inv in pairs(inventories) do
        count = count + inv.get_item_count({name=item_name, quality=_quality})
    end
    return count
end
...
function exchange_inventory(data)
    ...
    local entity_a = data.a.entity
    local entity_b = data.b.entity
    local a_is_vehicle = entity_a.type == "car" or entity_a.type == "spider-vehicle"
    local b_is_vehicle = entity_b.type == "car" or entity_b.type == "spider-vehicle"
    local vehicle_inventories = { -- by removal priority
      defines.inventory.burnt_result,
      defines.inventory.car_trunk,
      defines.inventory.car_ammo,
      defines.inventory.fuel
    }
    local inventories_to_try_a = a_is_vehicle and vehicle_inventories or {defines.inventory.chest}
    local inventories_to_try_b = b_is_vehicle and vehicle_inventories or {defines.inventory.chest}
    local existing_inventories_a = {}
    local existing_inventories_b = {}
    for _, inv_name in pairs(inventories_to_try_a) do
      local inv = entity_a.get_inventory(inv_name)
      if inv then table.insert(existing_inventories_a, inv) end
    end
    for _, inv_name in pairs(inventories_to_try_b) do
      local inv = entity_b.get_inventory(inv_name)
      if inv then table.insert(existing_inventories_b, inv) end
    end
    local itemdata_a = {}
    local itemdata_b = {}
    ...
    for item, qualities in pairs(itemdata_b) do
      for quality, comparators in pairs(qualities) do
        for comparator, signal_data in pairs(comparators) do
          local item_name = item

          local a_accepts = itemdata_a[item] and itemdata_a[item][quality] and itemdata_a[item][quality][comparator] and itemdata_a[item][quality][comparator].min or 0
          local b_target = itemdata_b[item][quality][comparator].min

          local a_items = count_inventories_items(existing_inventories_a, item, quality)
          local b_items = count_inventories_items(existing_inventories_b, item, quality)
          ...
          if transfer_b_a > 0 and b_is_vehicle and entity_b.burner then
            -- b is pushing
            local cats = entity_b.burner.fuel_categories
            local fuel_item = get_fuel.item(item_name, cats)
            if fuel_item then
              local item_fuel_value = fuel_item.fuel_value
              if    data.b.data.virtual
                and data.b.data.virtual["signal-minimum-fuel"]
                and data.b.data.virtual["signal-minimum-fuel"]["normal"]
                and data.b.data.virtual["signal-minimum-fuel"]["normal"]["="]
                and data.b.data.virtual["signal-minimum-fuel"]["normal"]["="].min
                and data.b.data.virtual["signal-minimum-fuel"]["normal"]["="].min > 0
              then
                  local min_fuel_value = data.b.data.virtual["signal-minimum-fuel"]["normal"]["="].min * 1000000
                  entity_b_total_fuel = get_fuel.total_fuel_value_from_inventories(existing_inventories_b, cats)
          ...
          elseif transfer_b_a < 0 and a_is_vehicle and entity_a.burner then
            -- a is pushing
            local cats = entity_a.burner.fuel_categories
            local fuel_item = get_fuel.item(item_name, cats)
            if fuel_item then
              local item_fuel_value = fuel_item.fuel_value
              if    data.a.data.virtual
                and data.a.data.virtual["signal-minimum-fuel"]
                and data.a.data.virtual["signal-minimum-fuel"]["normal"]
                and data.a.data.virtual["signal-minimum-fuel"]["normal"]["="]
                and data.a.data.virtual["signal-minimum-fuel"]["normal"]["="].min
                and data.a.data.virtual["signal-minimum-fuel"]["normal"]["="].min > 0
              then
                local min_fuel_value = data.a.data.virtual["signal-minimum-fuel"]["normal"]["="].min * 1000000
                entity_a_total_fuel = get_fuel.total_fuel_value_from_inventories(existing_inventories_a, cats)
          ...
          if transfer_b_a > 0 then
              ...
              pusher = entity_b
              pusher_inv = existing_inventories_b
          elseif transfer_b_a < 0 then
              ...
              pusher = entity_a
              pusher_inv = existing_inventories_a
          end


          if pusher and puller and items_to_transfer > 0 then
              local inserted_count = puller.insert({name=item_name, count=items_to_transfer, quality=quality})
              -- insert to entity not directly to inventory
              -- puts fuel and ammo in the right place.
              if inserted_count > 0 then
                  inventories_remove_items(pusher_inv, {name=item_name, count=inserted_count, quality=quality})
...
function unit_update_selection_graphics(unit)
  ...
  if unit.show_selection_to_players and table_size(unit.show_selection_to_players) > 0 then
    if not (unit.selection_graphic and unit.selection_graphic.valid) then
      local unit_type = unit_get_type(unit)
      ...
      unit.selection_graphic.players = unit.show_selection_to_players
    end
  else
    if (unit.selection_graphic and unit.selection_graphic.valid) then
      unit.selection_graphic.destroy()
...
function unit_update_state(unit)
    ...
    if unit.vehicle.burner then
      if  (not unit.vehicle.burner.currently_burning)
       or (unit.vehicle.burner.remaining_burning_fuel < unit.vehicle.burner.currently_burning.name.fuel_value * 0.02 + 1)
      then
    ...
    if unit.path and unit.mode ~= "passive" and unit_has_energy(unit) and (game.tick + unit.unit_id) % 5 == 0 then
      ...
      if storage.forces and storage.forces[force_name] and storage.forces[force_name].surface_paths
       and storage.forces[force_name].surface_paths[surface_index] and storage.forces[force_name].surface_paths[surface_index].paths[path_id] then
         -- path can be found
        local path = storage.forces[force_name].surface_paths[surface_index].paths[path_id]
        ...
        if path.waypoints and path.waypoints[waypoint_id] then
          ...
          if waypoint.type == "position" then
            ...
            if waypoint.direct_drive_through then req = req + 2 end
            if distance < req then
              ...
              if distance < (unit_type.is_walker and move_to_acceptable_range_walker or move_to_acceptable_range_car) then
                ...
                if unit_has_energy(unit) then
                    if unit_type.is_walker then
                      ...
                      if unit.path then
                        local path = Path.get_path(unit.vehicle.force.name, unit.vehicle.surface.index, unit.path.path_id)
                        if path then
                          local waypoint_id = unit.path.waypoint_id or 1
                          if path.waypoints and path.waypoints[waypoint_id] then
                            local waypoint = path.waypoints[waypoint_id]
                            if waypoint.direct_drive_through then
                              target_speed = 1000000000
                              unit.target_speed = target_speed
                            end
                          end
                          if path.speed_limit and path.speed_limit > 0 then
                            --vehicle real acceleartion can't be scaled
    ...
    elseif unit.mode == "unit" then -- use a unit navigator for pathfinding
      ...
      -- Do not request past map limits
      if continue and (Util.abs(unit.target_position.x) > 1000000 or Util.abs(unit.target_position.y) > 1000000) then
        continue = false
      end
      ...
      if continue then
          ...
          elseif not unit.navpath then
            ...
            local prototype = prototypes.entity[unit_type.vehicle_solid]
            ...
            storage.unit_by_path_request_id = storage.unit_by_path_request_id or {}
            storage.unit_by_path_request_id[path_request_id] = unit
        ...
        if unit_has_energy(unit) then
            ...
            if (game.tick + unit.unit_id * 133) % 300 == 0 then
                ...
                for _, enemy in pairs(enemies) do
                  -- This should replace the intended previous check. If there are bugs related to that in the future,
                  -- try replacing commandble.parent_group by commandable.is_unit_group (see 2.0.7 changelog)
                    if not enemy.commandable.parent_group then 
                        if not group then
...
function unit_set_command(data)
    ...
    if not unit and data.unit_id then
        unit = storage.unit.units[data.unit_id]
    end
...
-- storage.structure_unit_numbers[unit_number] = struct_id-- includes sub entities
-- storage.structures[struct_id] = {type="", entitiy=main_entity, sub={sub_entities}} -- only main entity
...
function struct_manage_entity(entity, event)
  ...
  if not storage.structures then storage.structures = {} end
  if not storage.structure_unit_numbers then storage.structure_unit_numbers = {} end
  if not storage.next_struct_id then storage.next_struct_id = 1 end

  if entity.name == "vehicle-depot" then
      local struct = {}
      ...
      storage.structure_unit_numbers[entity.unit_number] = struct.struct_id
      --entity.active = false
      -- create chest
      local chest = struct_create_or_revive(
          ...
          entity.force)
      struct.entity = chest
      storage.structure_unit_numbers[chest.unit_number] = struct.struct_id
      ...
      storage.structure_unit_numbers[combinator.unit_number] = struct.struct_id
  ...
  elseif entity.name == vehicle_deployer_type.struct_main then
      ...
      if(struct.sub.overlay.unit_number) then -- simple_entity
          storage.structure_unit_numbers[struct.sub.overlay.unit_number] = struct.struct_id
      end
      ...
      if vehicle_deployer_type.struct_belt then
          ...
          --storage.structure_unit_numbers[struct.sub.belt.unit_number] = struct.struct_id
      end
...
function struct_unmanage(struct)
    ...
    if struct.sub then
        for _, subentity in pairs(struct.sub) do
            if subentity.valid then
                if storage.structure_unit_numbers and subentity.unit_number then
                    storage.structure_unit_numbers[subentity.unit_number] = nil
                end
                subentity.destroy()
            end
            struct.sub[_] = nil
    ...
    if struct.entity and struct.entity.valid then
      if storage.structure_unit_numbers then
          storage.structure_unit_numbers[struct.entity.unit_number] = nil
      end
    end
    if storage.structures then
        storage.structures[struct.struct_id] = nil
    end
...
function struct_unmanage_entity(entity)
    ...
    if storage.structures then
      local struct_id = storage.structure_unit_numbers[entity.unit_number]
      if struct_id then
        struct_unmanage(storage.structures[struct_id])
      end
    end
end
...
function struct_get_circuit_inputs(combinator)
    if combinator and combinator.valid then
        local inputs = {}
        local network = combinator.get_circuit_network(defines.wire_connector_id.circuit_red)
        local network_found = false
        if network and network.signals then
            network_found = true
            for _, signal in pairs(network.signals) do
              Util.signal_container_add(inputs, {value=signal.signal, min=signal.count})
            end
        end
        network = combinator.get_circuit_network(defines.wire_connector_id.circuit_green)
        if network and network.signals then
            network_found = true
            for _, signal in pairs(network.signals) do
              Util.signal_container_add(inputs, {value=signal.signal, min=signal.count})
              ...
              for _, section in pairs(cb.sections) do
                for _, filter in pairs(section.filters) do
                  if filter.value then
                      Util.signal_container_add(inputs, filter)
                  end
                end
              end
            end
...
function struct_process_vehicle_deployer(struct)
    if struct.entity and struct.entity.valid then
        ...
        if struct.deploy_entity then
            ...
            for _, item_type in pairs(inventory.get_contents()) do
...
function struct_tick()
    if not storage.structures then return end
    for struct_id, struct in pairs(storage.structures) do
...
function add_tick_task(task)
    ...
    if not storage.tick_tasks then
      storage.tick_tasks = {}
      storage.tick_tasks_next_id = 1
    end
    task.task_id = storage.tick_tasks_next_id
    storage.tick_tasks_next_id = storage.tick_tasks_next_id + 1
    storage.tick_tasks[task.task_id] = task
end
...
function set_unit_data_from_combinator(unit_id, combinator)
    ...
    if unit and combinator and combinator.valid then
        ...
        for i = 1, cb.sections_count do
          local logi_section = cb.get_section(i)
          for _, filter in pairs(logi_section.filters) do
            if filter.value then
              Util.signal_container_add(inputs, filter)
            end
          end
...
function set_unit_data_to_combinator(unit_id, combinator)
  local unit = unit_by_unit_id(unit_id)
  ---@cast combinator LuaEntity -nil
  if unit and combinator and combinator.valid then
    local signal_container = unit.data

    local logi_point = combinator.get_or_create_control_behavior()
    ---@cast logi_point LuaConstantCombinatorControlBehavior
    local section_count = logi_point.sections_count
    for i =1, section_count do
      logi_point.remove_section(i)
    end
    local index = 1
    local no_group_logi_section = logi_point.add_section()
    for type, names in pairs(signal_container) do
      if type == "groups" then
        ---@TODO: Support Logistic Section Groups
        -- for group, _ in pairs(names) do
        --   logi_section = logi_point.add_section(group)
        -- end
      else
        for name, qualities in pairs(names) do
          for quality, comparators in pairs(qualities) do
            for comparator, data in pairs(comparators) do
              local filter
              if quality ~= data.value.quality then
                filter = {value = data.value.name, min=data.min}
              else
                filter = data
              end
              no_group_logi_section.set_slot(index, filter)
              index = index + 1
            end
          end
        end
...
function unit_on_player_enter_vehicle(player)
    ...
    if unit then
      ...
      if (not storage.player_selected_units)
        or (not storage.player_selected_units[player.index])
        or table_size(storage.player_selected_units[player.index]) < 1 then
          UnitRemote.select_units(player, {unit})
      end
    end
end
...
function unit_tick()
    ...
    for _, entity in pairs(storage.unit.entities_pending_manage) do
...
function on_tick()
  ...
  if storage.tick_tasks then
      for _, task in pairs(storage.tick_tasks) do
          local valid = false
          ...
          if not valid then storage.tick_tasks[task.task_id] = nil end
      end
  end
  ...
  if storage.robot_collections then
    for k, robot_collection in pairs(storage.robot_collections) do
      if not (robot_collection.vehicle and robot_collection.vehicle.valid) then
        storage.robot_collections[k] = nil
      elseif robot_collection.vehicle.logistic_cell and robot_collection.vehicle.logistic_cell.valid
       and robot_collection.vehicle.logistic_cell.logistic_network and robot_collection.vehicle.logistic_cell.logistic_network.valid then
        for _, robot in pairs(robot_collection.robots) do
          if robot.valid and robot.surface == robot_collection.vehicle.surface then
            robot.logistic_network = robot_collection.vehicle.logistic_cell.logistic_network
          end
        end
        storage.robot_collections[k] = nil
      end
...
function on_gui_opened(event)
  if event.entity and event.entity.valid and (event.entity.type == "car" or event.entity.type == "spider-vehicle") then
    ...
    if unit then
      if (not storage.player_selected_units)
        or (not storage.player_selected_units[player.index])
        or #storage.player_selected_units[player.index] < 1 then
          UnitRemote.select_units(player, {unit})
      end
    end
...
function on_load_unit_group(event, name)
  ...
  if player.cursor_stack and not (player.cursor_stack.valid_for_read and player.cursor_stack.name == 'unit-remote-control') then
...
function on_runtime_mod_setting_changed(event)
  if event.setting == "restrict-remote-actions-to-player-ids" then
    storage.restrict_remote_actions_to_player_ids = nil
     ...
     settings.global["restrict-remote-actions-to-player-ids"].value ~= "" then
       ...
       for _, word in pairs(words) do
         ...
         if number and number > 0 then
           storage.restrict_remote_actions_to_player_ids = storage.restrict_remote_actions_to_player_ids or {}
           storage.restrict_remote_actions_to_player_ids[number] = number
         end
       end
    end

    if storage.restrict_remote_actions_to_player_ids then
      local numbers_string = ""
      for _, number in pairs(storage.restrict_remote_actions_to_player_ids) do
...
function on_configuration_changed(data)
    storage.fuel_items = get_fuel.build()
    --storage.prototypes_require_load = true
    storage.unit_types = storage.unit_types or {}
    storage.unit_types_by_signal = storage.unit_types_by_signal or {}
    storage.unit_mineable_resources = storage.unit_mineable_resources or {}
    ...
    for _, unit in pairs(storage.unit.units) do
      if unit.data then
        -- unit.data = container[signal.type][signal.name][signal.quality][signal.comparator] = {value = signal, min = count}
        for type, names in pairs(unit.data) do
          for name, qualities in pairs(names) do
            -- verify it is valid
            for quality, comparators in pairs(qualities) do
              if not prototypes.quality[quality] then
                unit.data[type][name][quality] = nil
              end
            end
            if type == "virtual" then
              if not prototypes.virtual_signal[name] then
                unit.data[type][name] = nil
              end
...
function on_init()
    ...
    storage.unit = storage.unit or {}
    -- delayed buffer of on_built_entity so that the script can handle assignment if responsible for creation
    storage.unit.entities_pending_manage = storage.unit.entities_pending_manage or {}
    -- convert a unit_number to a unit_id
    storage.unit.unit_numbers = storage.unit.unit_numbers or {}
    storage.unit.next_unit_id = storage.unit.next_unit_id or 1
    ...
    unit_load_prototypes()
    storage.unit.units = storage.unit.units or {} -- by unit_id
...
function on_script_path_request_finished(event)
  if storage.unit_by_path_request_id and storage.unit_by_path_request_id[event.id] then
    local unit = storage.unit_by_path_request_id[event.id]
    storage.unit_by_path_request_id[event.id] = nil
      ...
      and unit.searching_for_path == event.id then -- make sure this is not a late path after another order was given
        ...
        if not event.try_again_later then
          ...
          if event.path then
            ...
            for _, player in pairs(unit.vehicle.force.connected_players) do
