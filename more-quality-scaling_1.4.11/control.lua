...
local function record_fluidbox(entity)
  if (entity == nil) or (not entity.valid) then return nil end
  local fb = entity.fluidbox
  if (fb == nil) or (#fb < 1) then return nil end
  local contents = {}
  for i = 1, #fb do
    local fluid = fb[i]
    if fluid then
      contents[i] = {name = fluid.name, amount = fluid.amount, temperature = fluid.temperature}
    end
  end
  return contents
end

local function restore_fluidbox(entity, contents)
  if (contents == nil) or (entity == nil) or (not entity.valid) then return end
  local fb = entity.fluidbox
  if (fb == nil) or (#fb < 1) then return end
  for i = 1, #fb do
    fb[i] = contents[i]
  end
end
...
on_built = function(data, now)
    ...
    local fluids = record_fluidbox(entity)
    local info = {
        name = name,
    ...
    elseif data.player_index and delayedPlacement[entity.type] and not now then -- don't delay replacement for robot/platform building
      ...
      if res and res.valid then
        restore_fluidbox(res, fluids)
      elseif (entity.type == "loader" or entity.type == "loader-1x1") then
        -- force-replace loaders if fast-replace isn't possible
        entity.destroy({raise_destroy=true})
        res = surface.create_entity(info)
        if res and res.valid then
          restore_fluidbox(res, fluids)
        end
      end
...
if defines.events.on_entity_quality_changed then
  script.on_event(defines.events.on_entity_quality_changed, function(event)
    if not (event and event.entity and event.entity.valid) then return end
    on_built({entity = event.entity, player_index = event.player_index}, true)
  end)
end
