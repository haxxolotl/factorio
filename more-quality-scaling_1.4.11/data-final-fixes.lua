...
if settings.startup["mqs-belt-changes"].value then
    ...
    -- keep loader energy/fluid requirements in sync with base prototypes
    for _, cat in pairs({"loader", "loader-1x1"}) do
        local raw = data.raw[cat]
        if raw then
            for name, original in pairs(getEntities(cat)) do
                for qname, _ in pairs(qualities) do
                    local qentity = raw[qname.."-"..name]
                    if qentity then
                        qentity.energy_source = original.energy_source and table.deepcopy(original.energy_source) or nil
                        qentity.fluid_box = original.fluid_box and table.deepcopy(original.fluid_box) or nil
                        qentity.fluid_boxes = original.fluid_boxes and table.deepcopy(original.fluid_boxes) or nil
                        if original.energy_per_item then
                            qentity.energy_per_item = original.energy_per_item
                        end
                        if original.energy_per_movement then
                            qentity.energy_per_movement = original.energy_per_movement
                        end
                    end
                end
            end
